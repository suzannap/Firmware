...
int POZYX::getWhoAmI(uint8_t *whoami , uint16_t remote_id)
{
  assert(whoami != NULL);
...
int POZYX::getFirmwareVersion(uint8_t *firmware, uint16_t remote_id)
{
...
int POZYX::getHardwareVersion(uint8_t *hardware, uint16_t remote_id)
{
...
int POZYX::getSelftest(uint8_t *selftest, uint16_t remote_id)
{
...
int POZYX::getErrorCode(uint8_t *error_code, uint16_t remote_id)
{
...
int POZYX::getInterruptStatus(uint8_t *interrupts, uint16_t remote_id)
{
...
int POZYX::getCalibrationStatus(uint8_t *calibration_status, uint16_t remote_id)
{
...
int POZYX::getInterruptMask(uint8_t *mask, uint16_t remote_id)
{
  assert(mask != NULL);
...
int POZYX::setInterruptMask(uint8_t mask, uint16_t remote_id)
{
  int status;
  if(remote_id == NULL){
...
int POZYX::getUpdateInterval(uint16_t *ms, uint16_t remote_id)
{
  assert(ms != NULL);
...
int POZYX::setUpdateInterval(uint16_t ms, uint16_t remote_id)
{ 
  assert(ms > 100);
  assert(ms <= 60000);
...
int POZYX::getConfigModeGPIO(int gpio_num, uint8_t *mode, uint16_t remote_id){
  assert(gpio_num > 0);
  assert(gpio_num <= 4);
  assert(mode != NULL);
...
int POZYX::getConfigPullGPIO(int gpio_num, uint8_t *pull, uint16_t remote_id)
{
  assert(gpio_num > 0);
  assert(gpio_num <= 4);
  assert(pull != NULL);
...
int POZYX::setConfigGPIO(int gpio_num, int mode, int pull, uint16_t remote_id)
{
  assert(gpio_num > 0);
  assert(gpio_num <= 4);
...
int POZYX::setLedConfig(uint8_t config, uint16_t remote_id)
{
  int status;
...
int POZYX::getPositionAlgorithm(uint8_t *algorithm, uint16_t remote_id)
{
...
int POZYX::getPositionDimension(uint8_t *dimension, uint16_t remote_id)
{
...
int POZYX::setPositionAlgorithm(int algorithm, int dimension, uint16_t remote_id)
{
...
int POZYX::getAnchorSelectionMode(uint8_t *mode, uint16_t remote_id)
{
  assert(mode != NULL);
...
int POZYX::getNumberOfAnchors(uint8_t *nr_anchors, uint16_t remote_id)
{
...
int POZYX::setSelectionOfAnchors(int mode, int nr_anchors, uint16_t remote_id)
{
...
int POZYX::getNetworkId(uint16_t *network_id)
{
...
int POZYX::setNetworkId(uint16_t network_id, uint16_t remote_id)
{
  assert(network_id != 0);
...
int POZYX::getUWBSettings(UWB_settings_t *UWB_settings, uint16_t remote_id)
{
...
int POZYX::setUWBSettings(UWB_settings_t *UWB_settings, uint16_t remote_id)
{
...
int POZYX::setUWBChannel(int channel_num, uint16_t remote_id)
{
...
int POZYX::getUWBChannel(int* channel_num, uint16_t remote_id)
{
...
int POZYX::setTxPower(float txgain_dB, uint16_t remote_id)
{
...
int POZYX::getTxPower(float* txgain_dB, uint16_t remote_id)
{
...
int POZYX::getOperationMode(uint8_t *mode, uint16_t remote_id)
{
  assert(mode != NULL);
...
int POZYX::setOperationMode(uint8_t mode, uint16_t remote_id)
{
...
int POZYX::getSensorMode(uint8_t *sensor_mode, uint16_t remote_id)
{
...
int POZYX::setSensorMode(uint8_t sensor_mode, uint16_t remote_id)
{
...
*
*/
int POZYX::getCoordinates(coordinates_t *coordinates, uint16_t remote_id)
{
...
int POZYX::setCoordinates(coordinates_t coordinates, uint16_t remote_id)
{
  int status;
...
int POZYX::getPositionError(pos_error_t *pos_error, uint16_t remote_id)
{
...
int POZYX::getAllSensorData(sensor_data_t *sensor_data, uint16_t remote_id)
{
...
int POZYX::getPressure_Pa(float32_t *pressure, uint16_t remote_id)
{
...
int POZYX::getAcceleration_mg(acceleration_t *acceleration, uint16_t remote_id)
{
...
int POZYX::getMagnetic_uT(magnetic_t *magnetic, uint16_t remote_id)
{
...
int POZYX::getAngularVelocity_dps(angular_vel_t *angular_vel, uint16_t remote_id)
{
...
int POZYX::getEulerAngles_deg(euler_angles_t *euler_angles, uint16_t remote_id)
{
...
int POZYX::getQuaternion(quaternion_t *quaternion, uint16_t remote_id)
{ 
...
int POZYX::getLinearAcceleration_mg(linear_acceleration_t *linear_acceleration, uint16_t remote_id)
{
...
int POZYX::getGravityVector_mg(gravity_vector_t *gravity_vector, uint16_t remote_id)
{
...
int POZYX::getTemperature_c(float32_t *temperature, uint16_t remote_id)
{
...
int POZYX::getDeviceListSize(uint8_t *device_list_size, uint16_t remote_id)
{
...
int POZYX::getLastNetworkId(uint16_t *network_id, uint16_t remote_id)
{
  assert(network_id != NULL);
...
int POZYX::getLastDataLength(uint8_t *data_length, uint16_t remote_id)
{
...
int POZYX::getGPIO(int gpio_num, uint8_t *value, uint16_t remote_id)
{
  assert(gpio_num >= 1);
  assert(gpio_num <= 4);
  assert(value != NULL);
...
int POZYX::setGPIO(int gpio_num, uint8_t value, uint16_t remote_id)
{
  assert(gpio_num >= 1);
  assert(gpio_num <= 4);
...
void POZYX::resetSystem(uint16_t remote_id)
{
  if(remote_id == NULL){
...
/*
String POZYX::getSystemError(uint16_t remote_id)
{
...
int POZYX::setLed(int led_num, boolean state, uint16_t remote_id)
{
  assert(led_num >= 1);
  assert(led_num <= 4);
...
int POZYX::doRanging(uint16_t destination, device_range_t *range)
{
...
int POZYX::doRemoteRanging(uint16_t device_from, uint16_t device_to, device_range_t* range)
{
...
int POZYX::doPositioning(coordinates_t *position, uint8_t dimension, int32_t height, uint8_t algorithm)
{
...
int POZYX::doRemotePositioning(uint16_t remote_id, coordinates_t *coordinates, uint8_t dimension, int32_t height, uint8_t algorithm)
{
  assert(remote_id != 0);
  assert(coordinates != NULL);
  assert( (algorithm == POZYX_POS_ALG_UWB_ONLY ) || (algorithm == POZYX_POS_ALG_LS) );
...
int POZYX::setPositioningAnchorIds(uint16_t anchors[], int anchor_num, uint16_t remote_id)
{
  assert(anchor_num > 0);
...
int POZYX::getPositioningAnchorIds(uint16_t anchors[], int anchor_num, uint16_t remote_id)
{
  assert(anchor_num > 0);
...
int POZYX::getDeviceIds(uint16_t devices[],int size, uint16_t remote_id)
{
  assert(size > 0);
...
int POZYX::getAnchorIds(uint16_t anchors[],int size, uint16_t remote_id)
{
  assert(size > 0);
  assert(size <= 20);
...
int POZYX::getTagIds(uint16_t tags[],int size, uint16_t remote_id)
{
  assert(size > 0);
  assert(size <= 20);
...
int POZYX::doDiscovery(int type, int slots, int slot_duration)
{
...
int POZYX::doAnchorCalibration(int dimension, int num_measurements, int num_anchors, uint16_t anchors[],  int32_t heights[])
{
...
int POZYX::clearDevices(uint16_t remote_id)
{
  int status;
...
int POZYX::addDevice(device_coordinates_t device_coordinates, uint16_t remote_id)
{
  int status;
...
int POZYX::getDeviceCoordinates(uint16_t device_id, coordinates_t *coordinates, uint16_t remote_id)
{
  assert(device_id != 0);
  assert(coordinates != NULL);
...
int POZYX::getDeviceRangeInfo(uint16_t device_id, device_range_t *device_range, uint16_t remote_id)
{
  assert(device_id != 0);
...
int POZYX::configInterruptPin(int pin, int mode, int bActiveHigh, int bLatch, uint16_t remote_id)
{
  assert( (pin <= 6) && (pin >= 0));
...
int POZYX::saveConfiguration(int type, uint8_t registers[], int num_registers, uint16_t remote_id)
{
...
int POZYX::clearConfiguration(uint16_t remote_id)
{
  int status;
  if(remote_id == NULL){
...
int POZYX::getNumRegistersSaved(uint16_t remote_id)
{
  int status;
...
bool POZYX::isRegisterSaved(uint8_t regAddress, uint16_t remote_id)
{
  int status;
...
void __attribute__((weak)) __assert (const char *func, const char *file, int line, const char *failedexpr)
{
  ...
  if (Serial){
    PX4_INFO("assertion in function : %s", func);
    PX4_INFO("assertion failed : %s", failedexpr);
    PX4_INFO("Filename: %s", file);
    PX4_INFO("Line number: %d", line);
...
void __attribute__((weak)) __assert_pozyx (const char *__func, const char *__file, int __lineno)
{
    // print out whatever you like here, function name, filename, line#, expression that failed.
  if (Serial){
    PX4_INFO("assertion failed in function : %s", __func);
    PX4_INFO("Filename: %s", __file);
    PX4_INFO("Line number: %d", __lineno);
